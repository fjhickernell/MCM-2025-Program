...
def extract_participants(dfs):
        ...
        for _, row in df.iterrows():
            ...
            if all(col in df.columns for col in ["First or given name(s) of presenter", "Last or family name of presenter"]):
                participants.append({
                    ...
                    "SessionTitle": row.get("SessionTitle", ""),
                    "PageNumber": "",
        ...
        for idx, row in df.iterrows():
            ...
            for i in range(1, 4):
                ...
                if organizer_col in df.columns and pd.notna(row[organizer_col]) and row[organizer_col]:
                    ...
                    if len(name_parts) >= 2:
                        ...
                        session_id = row.get("SessionID", "S")
                        session_title = row.get("SessionTitle", "")
                        participants.append({
                            "FirstName": firstName,
                            "LastName": lastName,
            ...
            if "Presenter 1 first or given name(s)" in df.columns and "Presenter 1 last or family name(s)" in df.columns:
                participants.append({
                    ...
                    "SessionID": row.get("SessionID", f"S"),
                    "SessionTitle": row.get("SessionTitle", ""),
                    "PageNumber": "",
        ...
        for _, row in df.iterrows():
            if all(col in df.columns for col in ["First or given name(s) of presenter", "Last or family name of presenter"]):
                ...
                participants.append({
                    "FirstName": row["First or given name(s) of presenter"],
                    "LastName": row["Last or family name of presenter"],
                    "SessionID": row.get("SessionID", "SS"),
                    "SessionTitle": row.get("Special Session Title", ""),
                    "PageNumber": "",
                    "Organization": row.get("Institution of presenter", "")
                })
    ...
    # if FirstName contains "-" and LastName contains "-", then remove the row
...
def validate_session_participants(participants_df):
    ...
    """
    grouped = participants_df.groupby("SessionID")
    validation_issues = []
...
if __name__ == '__main__':
    ...
    # Marge SessionID from merged_df into all dataframes in dfs using join_key
    for key in ["special_session_submissions", "plenary_abstracts", "contributed_talk_submissions"]:
        if key in dfs:
            dfs[key] = dfs[key].merge(merged_df[["join_key", "SessionID", "SessionTitle"]], how="left", on="join_key")
    
    # Step 6: Generate Participants.csv
    participants_df = extract_participants(dfs)
    validate_session_participants(participants_df)

    # Step 7: Order all required columns in Session
    merged_df = ensure_columns(merged_df, SessionListCols)

    # Step 8: Output final CSVs
