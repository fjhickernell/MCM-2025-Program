...
def process_talk(id_val: str, prefix: str, tex_dir: str, session_time:str, session_id:str) -> str | None:
    ...
    body_lines = [line.replace(bad_s, "").replace(bad_s2, "").replace('"', "''").replace(" &", " \&").replace("the the ", "the ").replace("$\cL_p$","$\mathcal{L}_p$").replace("\KSD", "\mathsf{KSD}").replace("Ra\'{u}l", "Ra\'ul") for line in body_lines]
